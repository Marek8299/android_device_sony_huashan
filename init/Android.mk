...
$(ramdisk_init_real): $(TOYBOX_STATIC) $(ramdisk_init) $(PRODUCT_OUT)/utilities/keycheck $(PRODUCT_OUT)/utilities/init_sony
	$(hide) if [ ! -d $(TARGET_RAMDISK_OUT)/bin ]; then \
		mkdir -p $(TARGET_RAMDISK_OUT)/bin; \
	fi
	cp $(TOYBOX_STATIC) $(TARGET_RAMDISK_OUT)/bin/toybox_init
